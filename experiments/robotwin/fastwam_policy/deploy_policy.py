...
def _parse_override_list(value: Any) -> list[str]:
    if _is_none_like(value):
        return []
...
def _compose_sim_cfg(
    ...
    for override in sim_model_overrides:
        if not override.startswith("model."):
            raise ValueError(
                f"`sim_model_overrides` only supports `model.*` overrides, got: {override}"
            )
...
def get_model(usr_args: Dict[str, Any]):
    ...
    sim_model_overrides = _parse_override_list(usr_args.get("sim_model_overrides"))
    cfg = _compose_sim_cfg(
        sim_cfg_path=sim_cfg_path,
        sim_cfg_name=sim_cfg_name,
        sim_task=sim_task,
        sim_model=sim_model,
        sim_model_overrides=sim_model_overrides,
    )
