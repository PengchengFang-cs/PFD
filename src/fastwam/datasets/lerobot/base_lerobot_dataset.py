...
SUPPORTED_DATASET_SCHEMAS = {"auto", "lerobot_v21_nested", "pi_libero_v20_flat"}
...
class BaseLerobotDataset(torch.utils.data.Dataset):
    ...
    def _resolve_dataset_schema(
        ...
        if dataset_schema not in SUPPORTED_DATASET_SCHEMAS:
            raise ValueError(
                f"Unsupported dataset_schema={dataset_schema!r}. "
                f"Expected one of {sorted(SUPPORTED_DATASET_SCHEMAS)}."
            )
        if dataset_schema != "auto":
            return dataset_schema

        if {"state", "actions"}.issubset(available_feature_keys):
            return "pi_libero_v20_flat"
        if "observation.state" in available_feature_keys and any(
            key == "action" or key.startswith("action.") for key in available_feature_keys
        ):
            return "lerobot_v21_nested"

        raise KeyError(
            "Cannot auto-detect dataset schema from available_feature_keys="
            f"{sorted(available_feature_keys)}"
        )

    @staticmethod
    def _schema_expected_lerobot_key(dataset_schema: str, modality: str, key: str) -> str:
        if dataset_schema == "lerobot_v21_nested":
            if modality == "image":
                return f"observation.images.{key}" if key != "default" else "observation.images"
            if modality == "state":
                return f"observation.state.{key}" if key != "default" else "observation.state"
            if modality == "action":
                return f"action.{key}" if key != "default" else "action"
        elif dataset_schema == "pi_libero_v20_flat":
            if modality == "image":
                return key if key != "default" else "image"
            if modality == "state":
                return key if key != "default" else "state"
            if modality == "action":
                return key if key != "default" else "actions"
        raise ValueError(
            f"Cannot map modality={modality!r}, key={key!r} with dataset_schema={dataset_schema!r}"
        )
    ...
    def __init__(
        ...
        available_feature_keys = set(metas[0].features.keys())
        for meta in metas[1:]:
            available_feature_keys.intersection_update(meta.features.keys())
        self.dataset_schema = self._resolve_dataset_schema(dataset_schema, available_feature_keys)
        logger.info(
            "Using dataset_schema=%s for dataset_dirs=%s",
            self.dataset_schema,
            self.dataset_dirs,
        )
        ...
        for meta in self.image_meta:
            key = meta["key"]
            meta["lerobot_key"] = self._schema_expected_lerobot_key(self.dataset_schema, "image", key)
            if meta["lerobot_key"] not in available_feature_keys:
                raise KeyError(
                    f"Missing image key `{meta['lerobot_key']}` for dataset_schema={self.dataset_schema}. "
                    f"Available keys: {sorted(available_feature_keys)}"
                )
        ...
        for meta in self.state_meta:
            key = meta["key"]
            meta["lerobot_key"] = self._schema_expected_lerobot_key(self.dataset_schema, "state", key)
            if meta["lerobot_key"] not in available_feature_keys:
                raise KeyError(
                    f"Missing state key `{meta['lerobot_key']}` for dataset_schema={self.dataset_schema}. "
                    f"Available keys: {sorted(available_feature_keys)}"
                )
            delta_timestamps[meta["lerobot_key"]] = [
                (t * global_sample_stride) / fps for t in range(-past_obs_size, -past_obs_size + obs_size)
        ...
        for meta in self.action_meta:
            key = meta["key"]
            meta["lerobot_key"] = self._schema_expected_lerobot_key(self.dataset_schema, "action", key)
            if meta["lerobot_key"] not in available_feature_keys:
                raise KeyError(
                    f"Missing action key `{meta['lerobot_key']}` for dataset_schema={self.dataset_schema}. "
                    f"Available keys: {sorted(available_feature_keys)}"
                )
