...
def _infer_legacy_stats_count(local_dir: Path) -> list[int]:
    if (local_dir / EPISODES_PATH).exists():
        episodes = load_jsonlines(local_dir / EPISODES_PATH)
        total = sum(int(item.get("length", 0)) for item in episodes)
        if total > 0:
            return [total]
    if (local_dir / INFO_PATH).exists():
        info = load_json(local_dir / INFO_PATH)
...
def _ensure_legacy_count_in_stats(stats: dict, local_dir: Path) -> dict:
    missing = []
    synthetic_count = _infer_legacy_stats_count(local_dir)
    for feature_name, feature_stats in stats.items():
        if not isinstance(feature_stats, dict):
            continue
        has_core_stats = {"min", "max", "mean", "std"}.issubset(feature_stats.keys())
        if has_core_stats and "count" not in feature_stats:
            feature_stats["count"] = list(synthetic_count)
            missing.append(feature_name)
    if missing:
        logging.warning(
            "Synthesized missing `count` stats for legacy dataset at %s. features=%s count=%s",
            local_dir,
            missing,
            synthetic_count[0],
        )
...
def load_stats(local_dir: Path) -> dict[str, dict[str, np.ndarray]]:
    ...
    stats = _ensure_legacy_count_in_stats(stats, local_dir)
