...
class LeRobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self._use_worker_local_hf_dataset = runtime_profile == "robotwin_worker_local_direct"
        self._use_direct_hf_indexing = self._use_worker_local_hf_dataset
        ...
        try:
            ...
            self.hf_dataset = None if self._use_worker_local_hf_dataset else self.load_hf_dataset()
        ...
        if self._use_worker_local_hf_dataset:
            logging.info(
                "Using runtime_profile=%s for repo_id=%s: lazy hf_dataset open with direct indexing hot path",
        ...
        # Check timestamps
        if not self._use_worker_local_hf_dataset:
    ...
    def _get_hf_dataset(self) -> datasets.Dataset:
        if self.hf_dataset is None:
            self.hf_dataset = self.load_hf_dataset()
        return self.hf_dataset

    @staticmethod
    def _stack_query_values(values):
        if torch.is_tensor(values):
            return values
        return torch.stack(list(values))

    @staticmethod
    def _timestamps_to_list(values) -> list[float]:
        if torch.is_tensor(values):
    ...
    def _get_hf_batch_by_indices(self, indices: list[int]):
        hf_dataset = self._get_hf_dataset()
        row_indices = list(indices)
        if self._use_direct_hf_indexing:
            return hf_dataset[row_indices]
        return hf_dataset.select(row_indices)
    ...
    def num_frames(self) -> int:
        ...
        if self._use_worker_local_hf_dataset:
            if self.episode_data_index is None or len(self.episode_data_index["to"]) == 0:
                return 0
    ...
    def _get_query_timestamps(
        ...
        if self._use_direct_hf_indexing:
            ...
            for key in self.meta.video_keys:
                if query_indices is not None and key in query_indices:
                    q_idx_tuple = tuple(query_indices[key])
                    if q_idx_tuple not in timestamp_cache:
                        selected_data = self._get_hf_batch_by_indices(list(q_idx_tuple))
                        timestamp_cache[q_idx_tuple] = self._timestamps_to_list(selected_data["timestamp"])
                    query_timestamps[key] = timestamp_cache[q_idx_tuple]
                else:
        ...
        for key in self.meta.video_keys:
            if query_indices is not None and key in query_indices:
                timestamps = self._get_hf_dataset().select(query_indices[key])["timestamp"]
    ...
    def _query_hf_dataset(self, query_indices: dict[str, list[int]]) -> dict:
        if self._use_direct_hf_indexing:
            return {
                key: self._stack_query_values(self._get_hf_batch_by_indices(q_idx)[key])
                for key, q_idx in query_indices.items()
                if key not in self.meta.video_keys
            }
        return {
            key: torch.stack(self._get_hf_dataset().select(q_idx)[key])
            for key, q_idx in query_indices.items()
            if key not in self.meta.video_keys
        }
        ...
        for key, q_idx in query_indices.items():
            if key not in self.meta.video_keys :
                ...
                if q_idx_tuple not in processed_indices:
                    if self._use_direct_hf_indexing:
                        selected_data = hf_dataset[list(q_idx_tuple)]
                    else:
                        selected_data = hf_dataset.select(q_idx)
                    index_to_selected[q_idx_tuple] = selected_data
                    processed_indices.add(q_idx_tuple)
                else:
                    selected_data = index_to_selected[q_idx_tuple]
                result[key] = self._stack_query_values(selected_data[key])
        return result
    ...
    def get_episode_data(self, episode_id: int) -> dict:
        ...
        selected_data = self._get_hf_dataset()[q_idx]
        res_keys = self.meta.features.keys() - set(self.meta.video_keys)
        res = {key : self._stack_query_values(selected_data[key]) for key in res_keys}
        return res
    ...
    def create(
        ...
        obj.runtime_profile = "default"
        obj._use_worker_local_hf_dataset = False
        obj._use_direct_hf_indexing = False
        return obj
