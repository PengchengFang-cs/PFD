...
class Wan22Core(torch.nn.Module):
    ...
    def save_checkpoint(self, path, optimizer=None, step=None):
        payload = {
            "dit": self._materialize_state_dict_for_save(self.dit.state_dict()),
            "step": step,
    ...
    def _materialize_checkpoint_value(value: Any) -> Any:
        ...
        if isinstance(value, dict):
            ...
            for key, item in value.items():
                materialized[key] = Wan22Core._materialize_checkpoint_value(item)
            return materialized
        if isinstance(value, list):
            return [Wan22Core._materialize_checkpoint_value(item) for item in value]
        if isinstance(value, tuple):
            return tuple(Wan22Core._materialize_checkpoint_value(item) for item in value)
        return copy.deepcopy(value)

    @staticmethod
    def _materialize_state_dict_for_save(state_dict: dict[str, Any]) -> dict[str, Any]:
        return Wan22Core._materialize_checkpoint_value(state_dict)
