...
class MoT(nn.Module):
    ...
    def get_expert_tail_blocks(self, expert_name: str, num_layers: int) -> list[nn.Module]:
        if expert_name not in self.mixtures:
            raise KeyError(f"Unknown expert `{expert_name}`. Available experts: {self.expert_order}")
        if num_layers < 0 or num_layers > self.num_layers:
            raise ValueError(
                f"`num_layers` for expert `{expert_name}` must be in [0, {self.num_layers}], got {num_layers}."
            )
        if num_layers == 0:
            return []
        expert = self.mixtures[expert_name]
        start_idx = self.num_layers - num_layers
        return list(expert.blocks[start_idx:])
    ...
    @staticmethod
    def _apply_action_layer_adapter_if_needed(
        layer_idx: int,
        hidden_states: torch.Tensor,
        action_timestep: Optional[torch.Tensor],
        action_layer_adapters: Optional[nn.ModuleDict],
    ) -> torch.Tensor:
        if action_layer_adapters is None:
            return hidden_states
        adapter_key = str(layer_idx)
        if adapter_key not in action_layer_adapters:
            return hidden_states
        adapter = action_layer_adapters[adapter_key]
        if action_timestep is None:
            raise ValueError("`action_timestep` is required when `action_layer_adapters` are provided.")
        return adapter(hidden_states, action_timestep)
    ...
    def forward_action_with_video_cache(
        ...
        for layer_idx in range(self.num_layers):
            ...
            x = self._apply_action_layer_adapter_if_needed(
                layer_idx=layer_idx,
                hidden_states=x,
    ...
    def forward(
        ...
        for layer_idx in range(self.num_layers):
            ...
            for name, seq_len in zip(self.expert_order, seq_lens):
                ...
                if name == "action":
                    updated_tokens = self._apply_action_layer_adapter_if_needed(
                        layer_idx=layer_idx,
                        hidden_states=updated_tokens,
                        action_timestep=action_timestep,
                        action_layer_adapters=action_layer_adapters,
                    )
